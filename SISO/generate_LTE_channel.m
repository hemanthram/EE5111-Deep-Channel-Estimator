...
% for ii = 1:n_chn_realizations
%     H_org(:,:,ii) = (1/sqrt(2))*randn(Nsub,Nsub) + sqrt(-1)*(1/sqrt(2))*randn(Nsub,Nsub);%relevant_channel_models;
%     Rhh = Rhh + (1/n_chn_realizations)*H_org(1:Nsub,1,ii)*H_org(1:Nsub,1,ii)';
% end
...
save('H_64x64_allSNR.mat','noiseless','noisy_0dB','noisy_5dB','noisy_10dB','noisy_15dB','noisy_20dB');
...
% semilogy(SNR_vec, (mse_dd_k8), '-+','LineWidth',2)
% semilogy(SNR_vec, (mse_dd_k16), '-+','LineWidth',2)
% semilogy(SNR_vec, (mse_dd_k32), '-+','LineWidth',2)
% semilogy(SNR_vec, (mse_dd_k64), '-+','LineWidth',2)
legend('LS', 'MMSE', 'MMSE G', 'k=8','k=16','k=32','k=64')